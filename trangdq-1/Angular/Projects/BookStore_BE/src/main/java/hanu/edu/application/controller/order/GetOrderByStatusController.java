...
public class GetOrderByStatusController {
    ...
    public ResponseEntity<Response> getByOrderStatus(@RequestParam String status, @PathVariable int page) {
        return ResponseCustomBuilder.get200ResponseWithData("Order of " + status + " gotten successfully!", getOrderByStatusService.getByOrderStatus(status, page, 20));
    }
    ...
    public ResponseEntity<Response> getUserOrdersByOrderStatus(@RequestParam String status, @PathVariable long id, @PathVariable int page) {
        return ResponseCustomBuilder.get200ResponseWithData("Order of user id #" + id + " with status " + status + " gotten successfully!", getOrderByStatusService.getUserOrdersByOrderStatus(id, status, page, 20));
    }
}
